...
export class GameState {
  ...
  _execute(move) {
    ...
    if (this.positionCount[key] >= 3) {
      this.status = 'draw'; this.selected = null; this.validMoves = []; return;
    }
    this._updateStatus();
    this.selected = null; this.validMoves = [];
  }
  ...
  undoMove(n = 2) {
    ...
    const actualN   = Math.min(n, this.history.length);
    const remaining = this.history.slice(0, -actualN);
    this.reset(this._startPieces);
    remaining.forEach(m => this._execute(m));
    return true;
  }
}
