...
import { GameState, BoardState } from '../game/chinese-game.js';
...
export default function ChineseGame() {
  ...
  async function saveLayout() {
    ...
    try {
      const key = 'chess_layouts_chinese';
  ...
  if (isSetupMode) {
    ...
    const sel  = setupPieceType ? `已选: ${setupPieceType}` : '点击棋盘放子，右键删除';
    statusText = `摆子模式 · ${side}  |  ${sel}`;
  } else if (aiThinking) {
    statusText = '🤖 AI 思考中...';
  } else {
    const who = gs.currentTurn === COLOR.RED ? '红方' : '黑方';
    ...
    if (gs.status === 'checkmate') {
      statusText = `✓ 将死！${gs.winner === COLOR.RED ? '红方' : '黑方'} 获胜！`;
    }
  }
    ...
    <div className="app">
      <header className="app-header">
        <div className="app-title">
          <span className="title-main">中国象棋</span>
          <span className="title-sub">标准棋局 · 9×10</span>
        </div>
        <GameNav />
      </header>
      ...
      <div className="game-layout">
        ...
        <aside className="side-panel">
          ...
          <div className="card">
            ...
            <div className="rules">
              <ul>
                <li><strong>将帅:</strong> 在九宫格内横竖走一格，不得照面</li>
                <li><strong>士:</strong> 九宫格内斜走一格</li>
                <li><strong>象:</strong> 斜走两格（田字），不可过河</li>
                <li><strong>馬:</strong> 先直走一格再斜走一格，马脚不可被阻</li>
                <li><strong>車:</strong> 横竖无限格</li>
                <li><strong>砲:</strong> 移动同車，吃子需跳过一子</li>
                <li><strong>兵:</strong> 过河前只能向前，过河后可横走</li>
              </ul>
            </div>
          </div>
        </aside>
